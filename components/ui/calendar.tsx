...
export type CalendarProps = React.ComponentProps<typeof DayPicker> & {
  modifiersStyles?: Record<string, (date: Date) => { className?: string }>
}

function Calendar({ className, classNames, showOutsideDays = true, modifiersStyles, ...props }: CalendarProps) {
  const modifiers = React.useMemo(() => {
    if (!modifiersStyles) return undefined

    const result: Record<string, Date[]> = {}
    for (const key in modifiersStyles) {
      result[key] = props.modifiers?.[key] || []
    }
    return result
  }, [modifiersStyles, props.modifiers])

  const modifiersClassNames = React.useMemo(() => {
    if (!modifiersStyles || !modifiers) return undefined

    const result: Record<string, string> = {}
    for (const key in modifiersStyles) {
      if (modifiers[key]) {
        const dates = modifiers[key]
        for (const date of dates) {
          const style = modifiersStyles[key](date)
          if (style.className) {
            result[key] = style.className
          }
        }
      }
    }
