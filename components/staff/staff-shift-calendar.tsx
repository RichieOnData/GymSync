...
export function StaffShiftCalendar({ staff }: StaffShiftCalendarProps) {
  ...
  const findConflicts = (shifts: any[]) => {
    ...
    for (const [date, dateShifts] of Object.entries(shiftsByDate)) {
      for (let i = 0; i < dateShifts.length; i++) {
        ...
        const start1 = new Date(`${shift1.date}T${shift1.start_time}`)
        const end1 = new Date(`${shift1.date}T${shift1.end_time}`)
        ...
        for (let j = i + 1; j < dateShifts.length; j++) {
          ...
          const start2 = new Date(`${shift2.date}T${shift2.start_time}`)
          const end2 = new Date(`${shift2.date}T${shift2.end_time}`)
  ...
  const handleAddShift = async () => {
    try {
      ...
      const formattedStartTime = formatTimeForDB(newShift.start_time)
      const formattedEndTime = formatTimeForDB(newShift.end_time)
      ...
      if (supabaseError) {
        ...
        toast({
          title: "Error",
          description: `Failed to add shift: ${supabaseError.message}`,
          variant: "destructive",
        })
        return
      }
      ...
      if (!data || data.length === 0) {
        toast({
          title: "Error",
          description: "No data returned after inserting shift",
          variant: "destructive",
        })
        return
      }
      ...
      toast({
        title: "Error",
        description: err instanceof Error ? err.message : "An unexpected error occurred while adding the shift.",
        variant: "destructive",
      })
    }
  }
  ...
  const handleResolveConflict = async (conflict: any) => {
    try {
      ...
      const { error: deleteError } = await supabase.from("shifts").delete().eq("id", shiftToRemove.id)
