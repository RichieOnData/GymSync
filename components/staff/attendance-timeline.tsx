...
export function StaffAttendanceTimeline({ staff, attendanceRecords = [] }: StaffAttendanceTimelineProps) {
  const [localAttendanceRecords, setLocalAttendanceRecords] = useState<any[]>(attendanceRecords)
  ...
  const fetchAttendanceRecords = async () => {
    try {
      ...
      setLocalAttendanceRecords(data || [])
      setLoading(false)
    } catch (err) {
  ...
  const exportToCSV = () => {
    if (!localAttendanceRecords.length) return

    // Create CSV content
    const headers = ["Date", "Staff", "Status", "Check In", "Check Out", "Duration", "Shift Type"]
    const rows = localAttendanceRecords.map((record) => [
      record.date,
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center">
        ...
        <div className="flex gap-2 flex-1">
          ...
          <Button
            ...
            onClick={exportToCSV}
            disabled={localAttendanceRecords.length === 0}
          >
      ...
      <Card className="bg-zinc-900 border-zinc-800">
        <CardHeader>
          ...
          <CardDescription>
            Showing {localAttendanceRecords.length} records from {format(dateRange.from, "MMM d, yyyy")} to{" "}
          ...
          {loading ? (
            ...
            <div className="overflow-x-auto">
              <Table>
                ...
                <TableBody>
                  {localAttendanceRecords.map((record) => {
